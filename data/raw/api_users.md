...
Available roles: `owner`, `admin`, `editor`, `viewer`. Role assignment requires `users:write` scope.

## Invitations

Invitations expire after 7 days. Resend via `POST /v1/users/invite/{id}/resend`.

## Deactivation

Deactivated users retain audit history but lose API access immediately.
## Best Practices

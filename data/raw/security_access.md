...
doc_type: security
plan_tier: all
product_area: access_control
doc_version: 1.0
---

# Access Control

RelayBoard implements defense-in-depth access controls.

## Multi-Factor Authentication

MFA can be enforced workspace-wide by admins. Supported methods: TOTP, WebAuthn, SMS (Enterprise).

## IP Allowlisting

Enterprise customers can restrict console and API access to specific IP ranges.

## Session Management

Sessions expire after 24 hours of inactivity. Admins can revoke all sessions for a user.

## API Key Rotation

Rotate API keys without downtime using the key rotation endpoint. Old keys remain valid for a configurable grace period.
## Best Practices
